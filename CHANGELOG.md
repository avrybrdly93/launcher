...
## 2026-08-31 (65th run) — **P0.110: the gate is now a script, not a list, and it caught two defects in its own commit**

- **P0.110 done.** `pnpm verify` runs every step `ci.yml` runs, in CI's order, and CLAUDE.md points at the script instead of naming four commands out of eleven. `packages/validation/src/pre-push-gate.test.ts` (8 assertions) fails when the two drift apart. `ROADMAP.json` carries the full record, as `policy.commitRules` requires, and this entry does not restate it.
- **What was actually missing, since the task title undersells it.** The title names the two typedoc steps, because those are what turned `main` red at a7f09b9. The documented gate was also missing **Format**, **Build app** and **Bundle size budget** — five of eleven steps, not two. A session could run every command CLAUDE.md named, see green, push, and land red on any of five.
- **The fix is deliberately not a longer list.** A list in prose is the thing that drifted: `ci.yml` gains a step, nobody edits the documentation, and the gap is invisible until it costs a red build. So the gate is one executable place, and the new test is the assertion that the two have not diverged — every `run:` step is in the script or in a named-with-a-reason omission list, the orders match, the script adds nothing CI lacks, and no omission entry names a step that no longer exists. **Mutation-checked rather than assumed:** dropping `engine run docs` from the script, and swapping `typecheck` after `lint`, each fail with the intended message.
- **Two omissions and one variant, all named rather than silently dropped.** `pnpm install --frozen-lockfile` and `playwright install` provision the environment rather than check the tree, and running either from a pre-push gate would be wrong rather than merely slow. The one intentional difference from CI is `check:cross-engine-drift` **without** `--record` — which is what `ci.yml`'s own comment on that step asks a local run to do.
- **The gate caught two defects in the commit that introduced it, which is the argument for the whole task.** First: `pnpm --filter @ballista/engine run docs` — `ci.yml`'s own form — is rejected by `root-scripts.test.ts`'s P0.90 guard, which forbids a space-separated pnpm value-flag in any root script. That guard is over-broad for `--filter` (whose value is present) but over-broad in the safe direction, so **it was not weakened**: the script uses `--filter=` and the new test canonicalises `--filter X` → `--filter=X` on both sides. Only `--filter` is rewritten — `--with-deps chromium firefox` takes a _list_, and a general rule would turn it into `--with-deps=chromium firefox`, a different command. Second: `tsc` rejected `match[1]` as possibly-undefined under `noUncheckedIndexedAccess` in the new test — **after vitest had already run it green**. A typecheck-after-test failure in the gate's own first run is precisely the class of thing the old four-command gate could not see.
- **Gate at the pushed tree**: `pnpm verify` end to end, all eleven steps, **exit 0**. Suite **3224 passed across 286 files** (the pre-change baseline measured this run was 3216 across 285; this run adds the 8 assertions in one file and nothing else). Bundle 73.1 kB gzipped against the 300 kB budget (§2.6).
- **A cost worth stating.** The gate is now materially slower than the four-command form — the suite alone is ~150 s and the app build ~28 s, so a full run is several minutes. That is the honest price of a gate that means what CI's green means, and it is why CLAUDE.md names one command rather than leaving a session to choose a subset.
- **Not claimed, and worth being explicit about.** `chunked-integration.test.ts`'s wall-clock budget (P0.112 / P0.96) did **not** fire locally in either full-suite run this session. That is two observations, not evidence the flake is gone; P0.112 stays open and un-narrowed. This run adds 8 assertions to one file with no timing in it.
- **Next run should read this before picking.** The three lowest-seq todos — **P0.99**, **P0.103**, **P0.101** — are all correctness bugs that return `ok: true` with the projectile underground, and the repo's priority order rightly puts them above anything by seq. **All three were already attempted by earlier runs and returned to todo**, and not for lack of effort: P0.99 needs the API change ADR-016 sketches and declines to choose between (the 27th run implemented both remedies its own notes proposed and measured both wrong — a throw fails 88 tests across 31 files); P0.103 and P0.101 both need a resting-contact model that blueprint 4.9 does not specify. **Taking one of them means making a modelling or API decision, not writing code**, and an unattended run that makes it unilaterally is how a fourth partial attempt lands. Either escalate for a decision, or take a completable task and say why — which is what this run did, recorded in P0.110's notes.
