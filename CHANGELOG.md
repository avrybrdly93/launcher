...
- **Both known flakes stayed green, reported as an observation and not as a fix.** P0.118's Playwright rejection and P0.123's 10 ms per-slice budget were both silent across this run's full suite. **Nothing was done to either task and neither is closed.** One local run is one sample; the filings stand exactly as written.
