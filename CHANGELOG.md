...
## 2026-09-05 (79th run) — **P0.118's application half fixed: the Plotly pane now owns its own mount/unmount ordering; the half that reddens CI turns out to be playwright-core's and is filed as P0.124**

- **`main` arrived at `a810a60`, the head this run starts from.** The arrival tree was gated before anything was changed, and that mattered: `pnpm test` alone reports **3 failures** in `cross-engine-drift-record.test.ts` on a fresh clone, because `packages/engine/dist/index.js` does not exist yet. They are a build-order artefact, not a defect — `pnpm typecheck` is `tsc -b`, which is what emits `dist/`, and CI runs it before `pnpm test`. **Run `pnpm typecheck` before `pnpm test` locally or you will chase three phantom failures.** In CI's order the arrival tree is **3613 passed / 304 files, exit 0**.
- **The write path was probed before the work, as the settled practice in this repository.** The claim commit went up on its own and proved the token before any effort was spent.
- **The task was taken OUT OF `seq` ORDER, and this run says so rather than quietly reordering** — which is exactly what the 78th run's handover asked of whoever took this decision. `policy.taskSelection` would select **P7.03** at seq 257; **P0.118** is at seq 316. The reason is the one that handover gave: `main` was red on CI, this filing was the entire cause, and every subsequent task's green-gate claim is unverifiable while that is true. A performance phase whose CI signal cannot be trusted is worse than a delayed performance phase.
- **What this environment could and could not validate was written into the claim commit, before any code, so it could not become a retrofit.** P0.118 has two manifestations. The original three-page-error signature is an application race. The one currently reddening CI is `FFPage._onWebSocketOpened`, which is **Firefox** — and this sandbox has no Firefox binary at all (`/opt/pw-browsers` holds only chromium variants and ffmpeg; `firefox.executablePath()` resolves to nothing that exists), so `tryLaunch` skips every Firefox case here. That half was therefore structurally unreproducible, and the claim note said so up front.
- **The application half was exactly the race the filing predicted, and it is fixed.** `renderLazyPlotlyPane` and `disposeLazyPlotlyPane` were two independent unawaited `async` calls with no knowledge of each other, invoked from a Preact effect whose cleanup is synchronous. Two losing orders, both producing the filed signature — `newPlot` resolving _after_ the route changed, so Plotly schedules an autoMargin redraw against a detached container; or `purge` running _before_ an in-flight `newPlot` finishes, so Plotly re-initialises state on a node just retired. **Neither is fixable from the component**, because the cleanup is synchronous and the work it cleans up is not, so the ordering now lives in the pane module: a per-container promise queue serialises every operation, and a generation token lets a render queued before an unmount decline to mount anything rather than mounting and being purged.
- **Two further defects on the same code path went with it.** `disposeLazyPlotlyPane` called `loadPlotlyModule()` unconditionally, so unmounting a pane that had never opened would **initiate the ~4.8 MB dynamic import in order to purge nothing** — defeating the lazy boundary the module's own doc comment is about. And `LazyPlotlyView` `void`-ed both promises, so a genuine Plotly failure both vanished _and_ resurfaced later as an unattributed unhandled rejection; failures now go to `console.error`, which `app-routes.e2e.test.ts` already asserts is empty, so a real one fails a test that names the route.
- **Teeth by reversion, not by assertion count.** The module was reverted and the suite re-run: **3 of the 6 new cases fail against the old implementation**, all 10 pass against the new one. One of them had _no_ teeth as first written — `disposeLazyPlotlyPane` suspends on its first `await` and returns, so releasing the stalled `newPlot` on the next line beat even the unsequenced version to the queue and the test passed against the very defect it existed to catch. It needed a microtask drain between the dispose and the release, and the assertion now says so in place.
- **One existing assertion was rewritten rather than deleted, and the distinction is the point.** It required dispose on an unrendered container to "load Plotly then call purge" — which encoded the second defect above _as a requirement_. Rewritten to the contract it should have had, with the history in the test's own comment. No test was skipped, disabled, weakened or serialised.
- **The half that actually reddens CI is not this repository's to fix, and that was established by reading source rather than by guessing.** `playwright-core`'s `FFPage._onWebSocketOpened` does `assert(this._webSocketRequests.get(event.requestId))`, then the same for the responses map. Both are filled by the earlier network events for the WebSocket upgrade, so when the `webSocketOpened` event arrives without its recorded request — a page teardown racing the handshake, which this suite provokes on **every** case by closing its page in a `finally` — the assert throws **inside an event handler** and escapes as an unhandled rejection. That is precisely why CI reports every test green and still exits 1, and why it is Firefox-only.
- **Three measurements were taken so the next run does not re-derive them, and two of them kill the obvious ideas.** (1) The assert is **byte-identical in playwright-core 1.63.0**, the latest stable, checked by unpacking the published tarball — so **bumping from the pinned `^1.61.1` does not fix it**. (2) **Disabling HMR does not fix it**: the WebSocket is Vite's dev-client connection and the client opens one regardless of the server. Measured against this app on Chromium — default **1** websocket, `server.hmr: false` still **1**, `server.ws: false` **2**, because the server refuses and the injected client retries. Vite's `ws: false` disables the _server_ socket, not the page's attempt. (3) No Firefox here, so no fix for it could have been validated locally in any case. **This is why no speculative fix for that half was pushed**: the claim commit said validation was impossible in this environment, and shipping a guess anyway would have made that note a formality.
- **Filed as P0.124** rather than left inside P0.118, because the two halves have different owners. Its notes carry the diagnosis, the three measurements, and the one remaining lever — having the route suite build and preview instead of driving a dev server, which is the only way to serve a page that opens no WebSocket. That is a **design decision** the suite's own doc comment argues against on purpose (the two suites drive different servers so a route working under one and not the other is itself a finding), so it is filed for someone to make deliberately rather than slipped in as a flake workaround. Dropping Firefox, serialising the suite or filtering the message are all explicitly off the table.
- **P0.118 stays `in-progress`, and its criterion is not met.** Three consecutive full-suite runs after the fix: **3619 passed / 304 files, exit 0 every time**, with **zero** occurrences of the `_redrawFromAutoMarginCount` signature and zero unhandled rejections. That is three runs, not the twenty the criterion asks for, and every Firefox case was skipped in all three. Claiming closure on that evidence would be exactly the kind of thing this file exists to prevent.
- **Full gate**: `pnpm typecheck` clean; `pnpm lint:deps` clean (**1735** modules, 4929 dependencies); `pnpm lint` carries only the **one pre-existing** unused-import warning in `golden-mc-results.test.ts`, unchanged from arrival; `prettier --check` clean; **3619 passed / 304 files**, up from 3613 (+6 cases, no new file). No benchmark was run and no performance claim is made anywhere in this entry.
- **Branch note, recorded because it departs from this repository's `CLAUDE.md`.** That file says to commit and push directly to `main` and not to leave `claude/*` branches behind. This run was launched with an explicit branch assignment — develop and push to `claude/nice-keller-qozzup`, and never push elsewhere without permission — which is a harness-level instruction this run is not free to override. So the work is on that branch and `main` is untouched. **Someone with the authority to merge should fast-forward `main` to it and delete the branch**; the two instructions genuinely conflict and the conservative reading was taken, since pushing to `main` is the irreversible half.
- **Next: P0.124 or P7.03, and the choice is now a real one rather than forced.** With the application half fixed, `main`'s redness is entirely an upstream driver bug that no amount of care in this repository will remove, so **P7.03 is no longer blocked on anything this repository can do** — take it in `seq` order unless someone wants to make P0.124's build-and-preview trade first. Two things to read before P7.03, carried forward verbatim from the 78th run because they have not been acted on: **(1)** bit-identity stops being structural there — reassociating the stage sum across replicates is exactly what changes rounding, and `explicit-rk-kernel.ts`'s doc comment already spells out the per-component operation order that must be preserved (`y[i] + h * (b1*k1[i] + b2*k2[i])`, **not** `(y[i] + h*b1*k1[i]) + h*b2*k2[i]`). **(2)** P0.123 proposes calibrating a timing budget against a same-process reference loop rather than a wall-clock constant; Phase 7 is where that starts to matter, so read it before writing any new timing assertion.
- **One arrival-gate finding worth carrying, unrelated to the task.** The `pnpm typecheck`-before-`pnpm test` ordering above is not written down anywhere in `CLAUDE.md`, which lists the CI commands but not the dependency between them. A run that follows that list in the order given still hits the three phantom failures. Not filed as a task — it is one sentence in a doc — but the next run to touch `CLAUDE.md` should add it.
