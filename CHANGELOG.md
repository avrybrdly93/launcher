...
## 2026-09-14 (104th run) — **P7.19 was already met and nothing asserted it, and the reason that mattered is that the gate everyone would assume covers it does not: two budgets on one channel in different units do not nest, and which is tighter flips at 64 m**

- **`main` arrived shallow again, and the lesson four runs have now recorded was applied before anything was believed.** `git rev-parse --is-shallow-repository` returned `true`; after `git fetch --unshallow`, HEAD was **identical to `origin/main`** at `cece920`, **0 ahead and 0 behind**. Before unshallowing the fetch had again reported a **FORCED UPDATE** on `origin/main`. **That is now five consecutive runs where the shallow clone answered confidently and wrongly.** Unshallow before believing any ahead/behind number in this repo.
- **Baseline captured to a file before any edit**: `pnpm typecheck` clean, `pnpm test` **332 files / 4113 passed / 2 skipped**, matching the 103rd run's close-out exactly. **No flake.**
...
- **THE READING WAS SETTLED FIRST, IN A COMMIT CONTAINING NO CODE** (`d07107f`), as runs 100-103 did. Three ambiguities decided in `ROADMAP.json` with reasons: **what "impact x" is** — neither the kernel nor the CPU reducer emits an absolute abscissa, both emit `range = |impactX - x0|`, and the two coincide only because every scenario here launches from `x0 = 0`, which is a property of **the ensemble and not of the code** and would have to be revisited for a non-zero launch abscissa; **what "of CPU" is** — the f32 CPU arm over the same f32 fixed-step trajectory, P7.16's reading (iii), because differencing against f64 or against the adaptive event-localised observables would move arithmetic width and event policy at once; and **that the bar is absolute metres**, not ULP and not relative, which at O(100) m ranges is a materially different criterion.
- **THE TASK WAS REVIEW-SHAPED DESPITE ITS TAG, AND SAYING SO IS THE HONEST PART.** The feature the title names — an in-kernel, fixed-iteration, branch-uniform bisection for the ground crossing — **already existed**, delivered by P7.16 as part of its on-device reduction: `WGSL_IMPACT_BISECTION_STEPS = 60`, hoisted out of the step loop so every thread runs exactly one bisection of exactly the same length, every conditional a `select`, no reliance on trap behaviour for `sqrt` of a negative or a zero denominator. **Nothing about the kernel needed to change and nothing did.** Writing a second bisection to have something to commit would have been manufacturing work.
- **AND THE CRITERION WAS ALREADY MET — AT 0 m — BY A RUN ALREADY IN THE REPOSITORY.** `scripts/gpu-observables-results.json`, recorded by P7.16: 10000 trajectories, h=0.001, 12000 steps, `range` **bit-identical on all 10000 values, `maxAbs` 0**, all 10000 flights landed so the bisection was genuinely exercised rather than compared false-against-false. **No new device measurement was taken and none was invented** — there is no adapter in this container. Provenance is unchanged from P7.16 and is **software**: vendor `google`, architecture `swiftshader`, Chromium 141.0.7390.37. Correctness only; throughput is P7.20's and needs hardware.
- **SO THE WHOLE CONTENT OF THE RUN IS THAT NOTHING ASSERTED IT — AND THE REASON THAT IS NOT PEDANTRY IS THE MOST REUSABLE THING HERE. THE GATE THAT LOOKS LIKE IT COVERS THIS ONE DOES NOT.** `check:gpu-observables` gates on a **256-ULP** budget, and the instinct is that a budget that tight subsumes a millimetre bar. The arithmetic says otherwise, exactly: 256 ULP of a binary32 value is at most 1e-3 m **only while the value is below 64 m**. On [32, 64) one ULP is 3.815e-6 and 256 of them are **9.77e-4 m**, just inside. On [64, 128) one ULP doubles to 7.629e-6 and 256 of them are **1.953e-3 m**, nearly twice the bar. Above 128 m it doubles again. **This ensemble's ranges are O(100) m** — squarely in the regime where the existing gate is the **weaker** of the two. A future change could pass `check:gpu-observables` and violate P7.19, and nothing in the repository would have noticed.
- **THE GAP IS EXHIBITED RATHER THAN DESCRIBED.** Mutating the recorded `range.maxAbs` to **1.5e-3 m** — which at a 100 m range is **197 ULP, comfortably inside the 256 budget** — fails the new assertion and would have passed the old gate alone. That is the failure mode, made concrete. (The mutation also trips the pre-existing identity-count consistency test, which is the results guard doing its job.)
- **AND THE NEW GATE HAS TEETH ON THE OTHER SIDE TOO.** Control C2 — `theta = 0.5` in place of the 60-iteration bisection, i.e. deleting the feature P7.19 names — measures **1987 ULP on range**, which at O(100) m is **1.5e-2 m, fifteen times this bar**. The gate rejects the deletion of the thing it gates, which is the property `ULP_BUDGET`'s own derivation had to pass and which a gate inherited by assumption would never have been checked for.
- **WHAT LANDED:** `IMPACT_X_ABS_BUDGET_M = 1e-3` as a **second, independent hard gate** in `scripts/measure-gpu-observables.mjs` (so a device run now re-validates P7.19 as well as P7.16, and writes a `p719` block into the results file); five assertions in `gpu-observables-results.test.ts` evaluating P7.19 against the already-recorded numbers; and **the non-nesting asserted as arithmetic** — 63.9 m inside, 64 m outside, 100 m at 1.953e-3 — so the two gates cannot later be collapsed into one by somebody reasoning from intuition about which sounds stricter.
- **Nothing in the test suite reads the `p719` block**, because the committed results file predates it and reading a field that is not there would mean either a test that cannot run or a field invented to satisfy it. Every assertion is derived from `perObservable`, which the recorded run did produce.
- **Gate green, every static step of `ci.yml` run locally**: `pnpm typecheck`; `pnpm lint` **0 errors** (1 pre-existing warning, `McStats` in `golden-mc-results.test.ts`, untouched); `pnpm format:check` clean; `pnpm lint:deps` **1892 modules / 5343 dependencies**, no violations; `pnpm test` **4118 passed / 2 skipped across 332 files** (+5 tests, +0 files on the 103rd run's recorded 4113/2/332 — the new cases were appended to an existing file); `pnpm check:backend-equivalence` **19 passed**; `pnpm --filter @ballista/app build`; `check-bundle-size` **93.8 kB**, unchanged.
- **The GPU checks COULD NOT BE RUN and that was verified rather than assumed.** `pnpm check:gpu-observables` was executed: it exits 0 while printing that Playwright has no browser installed and **"This is not a pass."** Same for `check:cross-engine-drift` and `check:gpu-rk4-agreement`. **Still open and still cheap on any machine with a real GPU:** `pnpm check:gpu-observables` now enforces both gates in one command, and `pnpm bench:gpu-workgroup --record` still adds a hardware row.
...
- **Next run: P7.20, at seq 274** — the throughput benchmark, ≥1e6 trajectories/s on a short-flight RK4 scenario, validation "measured number published with adapter info; budget §2.6 assessed". Two things it inherits. First, and it should be faced before any code: **P7.20 cannot be closed in this container at all.** Every previous GPU task closed here was a _correctness_ task, for which a SwiftShader adapter is legitimate evidence; a throughput number measured on SwiftShader is not evidence about anything and must not be recorded as if it were. Expect to either find hardware or leave P7.20 `in-progress` with the harness built and the measurement explicitly absent — and say which, rather than publishing a software-adapter figure with a caveat attached. Second, the part worth carrying furthest: **this run's task was already satisfied, and the work was finding out that the gate everyone would assume covered it did not.** The general form: **two budgets on the same channel in different units do not nest, and which is tighter depends on the magnitude of the quantity** — a relative bound and an absolute bound cross at a binade boundary. Anywhere this repository gates one channel in two units, check the ordering with the arithmetic rather than with intuition.
