...
- Before pushing to `main`, run the project's typecheck/lint/test suite
  locally (see `.github/workflows/ci.yml` for the exact commands:
  `pnpm typecheck`, `pnpm lint`, `pnpm lint:deps`, `pnpm test`, build) so
  broken code doesn't land un-vetted. CI still runs on every push to `main`
  as a backstop.
