import { createEnergyBalanceInvariant, createMechanicalEnergyInvariant } from "./energy.js";
...
export function createPlanarProjectileModel(forces: readonly ForceModel[]): Model {
  ...
  function rhs(t: number, y: Float64Array, out: Float64Array, ctx: EvalContext): void {
    const x = y[X]!;
    const yPos = y[Y]!;
    const vx = y[VX]!;
    const vy = y[VY]!;
  ...
  const model: Model = {
    ...
    rhs,
    invariants: [createMechanicalEnergyInvariant(), createEnergyBalanceInvariant(rhs, 4, registry)],
  };
