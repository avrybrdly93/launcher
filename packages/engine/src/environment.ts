...
/**
 * Isothermal exponential atmosphere: rho(y) = rho0*e^(-y/H), scale height
 * H = Rs*T0/g (§3.4 default: H ~ 8.5 km). Temperature is held at `T0`
 * (isothermal), so pressure follows the same exponential via the ideal gas
 * law (p = rho*Rs*T) and viscosity/speed-of-sound are the constant values
 * at `T0`.
 */
...
/** Uniform steady wind, slider-controlled: w = (wx, wy) constant everywhere (§3.5 case 1). */
...
/**
 * Sinusoidal gust: wx(t) = mean + amplitude*sin(omega*t + phase) (§3.5 case
 * 3), smooth by construction so solver convergence studies on this wind
 * stay clean.
 */
