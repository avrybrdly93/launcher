 ...
 * height H = Rs*T0/g ~ 8.5 km. Temperature is constant by the isothermal
 * assumption, so eta and c stay at their sea-level values — only rho (and,
 * consistently for an isothermal ideal gas, p) vary with altitude.
 */
export class ExponentialAtmosphere implements Atmosphere {
  ...
  constructor(
    ...
    scaleHeight: number = ISA.scaleHeight,
  ) {
...
/** Uniform steady wind, slider-controlled (§3.5 case 1): w = (wx, wy) everywhere, for all time. */
