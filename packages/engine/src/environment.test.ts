...
describe("ExponentialAtmosphere", () => {
  ...
  it("is rho0/e at y=H (P1.27 validation criterion) to 1e-15", () => {
    const atm = new ExponentialAtmosphere();
    const out = new EnvSample();
  ...
  it("decays monotonically with height and stays isothermal", () => {
    const atm = new ExponentialAtmosphere();
    const outLow = new EnvSample();
    const outHigh = new EnvSample();
    atm.sample(0, 1000, outLow);
    atm.sample(0, 5000, outHigh);
    expect(outHigh.rho).toBeLessThan(outLow.rho);
    expect(outLow.T).toBe(ISA.T0);
    expect(outHigh.T).toBe(ISA.T0);
    expect(outLow.c).toBe(outHigh.c);
  });

  it("is consistent with the ideal gas law p = rho*Rs*T", () => {
    const atm = new ExponentialAtmosphere();
    const out = new EnvSample();
