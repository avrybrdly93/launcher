...
describe("SinusoidalGustWind", () => {
  it("matches w(t) = mean + amplitude*sin(omega*t + phase) at sampled t", () => {
    ...
    for (const t of [0, 0.5, 1, 3.14159, 10, -2.5]) {
  ...
  it("defaults phase to 0", () => {
    const wind = new SinusoidalGustWind(0, 1, 2);
    const out = new EnvSample();
    wind.sample(0, 0, 0, out);
    expect(out.wx).toBeCloseTo(Math.sin(0), 15);
  });

  it("is independent of position (only t drives the gust)", () => {
    const wind = new SinusoidalGustWind(1, 2, 3, 0.1);
    const out = new EnvSample();
    wind.sample(5, 0, 0, out);
    const wAtOrigin = out.wx;
    wind.sample(5, 1000, -1000, out);
    expect(out.wx).toBe(wAtOrigin);
  });
});
